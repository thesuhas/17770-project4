...
impl<'a> Rewriter<'a, AnalysisData> {
    pub fn new(analysis: &'a mut Analysis<AnalysisData>) -> Self {
        ...
        Self {
            analysis,
            struct_allocs: vec![StructAlloc { local_offset: 0, num_fields: 0 }; num_allocations],
        }
    }

    pub fn rewrite(&mut self, module: &mut Module) {
        let mut modifier = module.functions.get_fn_modifier(self.analysis.func_id).unwrap();
